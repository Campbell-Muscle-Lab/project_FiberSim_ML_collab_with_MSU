...
def summary_figure(top_data_folder, sim_file_string, analysis_folder):
    ...
    for (i,d) in enumerate(data_dirs):
        ...
        ax[f_row - 1].plot(d['time'], d['hs_1_force'], '-')

        ax[a_row - 1].plot(d['time'], d['hs_1_a_pop_1'], 'r-')
        ax[a_row - 1].plot(d['time'], d['hs_1_a_pop_2'], 'g-')

        ax[m_row - 1].plot(d['time'], d['hs_1_m_pop_1'], 'r-')
        ax[m_row - 1].plot(d['time'], d['hs_1_m_pop_2'], 'g-')
        ax[m_row - 1].plot(d['time'], d['hs_1_m_pop_3'], 'b-')
        ax[m_row - 1].plot(d['time'], d['hs_1_m_pop_4'], 'm-')

        ax[c_row - 1].plot(d['time'], d['hs_1_c_pop_1'], 'r-')
        ax[c_row - 1].plot(d['time'], d['hs_1_c_pop_2'], 'g-')
        ax[c_row - 1].plot(d['time'], d['hs_1_c_pop_3'], 'b-')

        if (i==0):
            ax[Ca_row - 1].set_ylabel('Ca2+\n[M]')
            ax[hs_length_row - 1].set_ylabel('Half-sarcomere\nlength\n(µm)')
            ax[f_row - 1].set_ylabel('Sarcomere\nstress\n(N m^{-2})')
            ax[a_row - 1].set_ylabel('Active\nActin')
            ax[m_row - 1].set_ylabel('Bound\nmyosin')
            ax[c_row - 1].set_ylabel('MyBP-C')

            ax[c_row - 1].set_xlabel('Time_s')
